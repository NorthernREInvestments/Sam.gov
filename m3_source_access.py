...
def source_access_queue(opportunities: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in opportunities:
        ...
        if access not in {SA_AUTH_REQUIRED, SA_REGISTRATION_REQUIRED}:
            continue
        ...
        b = buckets.setdefault(
            source,
            {
                ...
                "operator_action_required": "ENTER_CREDENTIALS_OR_REGISTER",
        ...
        if not b["registration_url"]:
            b["registration_url"] = row.get("registration_url") or row.get("detail_url") or row.get("source_url")
        if not b["login_url"]:
            b["login_url"] = row.get("login_url") or b["registration_url"]
