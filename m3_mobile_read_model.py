...
def _deal_product(row: dict[str, Any]) -> dict[str, Any]:
    try:
        from m3_product_pricing import deal_room_product_section

        return deal_room_product_section(row)
    except Exception:
        return {
...
def _deal_document_intelligence(row: dict[str, Any]) -> dict[str, Any]:
    try:
        from m3_document_intelligence import deal_room_document_intelligence_section

        return deal_room_document_intelligence_section(row)
    except Exception:
        return {
...
def _deal_government_price_history(row: dict[str, Any]) -> dict[str, Any]:
    try:
        from m3_government_revenue_benchmark import deal_room_government_price_history_section

        return deal_room_government_price_history_section(row)
    except Exception as exc:
        return {
...
def _deal_public_pricing_evidence(row: dict[str, Any]) -> dict[str, Any]:
    try:
        from m3_public_pricing_evidence import deal_room_public_pricing_section

        return deal_room_public_pricing_section(row)
    except Exception:
        return {
            "kind": "M3DealRoomPublicPricingEvidence",
            "NEXT_MISSING_EVIDENCE": "Public pricing evidence unavailable",
            "DEVELOPMENT_NO_OUTREACH": True,
        }


def _deal_evidence_recovery(row: dict[str, Any]) -> dict[str, Any]:
    try:
        from m3_document_evidence import deal_room_evidence_recovery_section

        return deal_room_evidence_recovery_section(row)
    except Exception:
        return {
