...
function renderCsvUploadSummary(data) {
  return `
    ...
    ${
      data.attachments_deferred_background
        ? `<p class="detail-note csv-upload-budget-note">Import saved. SAM attachment downloads (if any) continue in the background and do not block this upload.</p>`
        : (data.attachments_waiting_for_budget ?? data.attachments_skipped_budget ?? 0) > 0
          ? `<p class="detail-note csv-upload-budget-note">Some attachment downloads are waiting for SAM API budget — dashboard pulls continue as credits allow.</p>`
          : ""
    }
...
function openCsvUploadModal() {
  ...
  document.getElementById("modal-content").innerHTML = `
    <div class="csv-upload-modal">
      ...
      <p class="detail-note">Upload <strong>ContractOpportunitiesFullCSV.csv</strong> (max 250 MB). Rows merge immediately; USAspending pricing runs on new/changed rows. SAM attachment downloads happen separately for dashboard contracts (and CSV queue in background) as API credits allow.</p>
