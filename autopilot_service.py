...
from api_budget import ai_intake_allowed, scheduled_sync_attachments_only
...
def get_autopilot_status() -> dict[str, Any]:
    ...
    out["ai_intake_allowed"] = ai_intake_allowed()
...
def run_stored_pdf_repair_pass() -> dict[str, Any]:
    ...
    if not ai_intake_allowed():
        return {"skipped": True, "reason": "ai_intake_disabled"}
    ...
    try:
        ...
        _set_status(running=False, finished_at=now_utc().isoformat())


def start_autopilot(*, trigger: AutopilotTrigger = "deploy") -> bool:
    """Background: pricing backfill, then repair every contract that still needs AI work."""
    global _running
    with _lock:
        if _running:
            return False
    ...
    def _run() -> None:
        global _running
        try:
            if trigger == "deploy" and not ai_intake_allowed():
                logger.info("Autopilot deploy: intake disabled — skipping AI repair")
                return
