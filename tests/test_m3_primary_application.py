...
def test_health_and_m3_health_endpoints(monkeypatch):
    ...
    assert "20260917-m3-discovery2" in body["build_version"]
...
def test_cache_bust_matches_build():
    html = (Path(__file__).resolve().parents[1] / "static" / "index.html").read_text(encoding="utf-8")
    assert "20260917-m3-discovery2" in html
    assert 'meta name="m3-build" content="20260917-m3-discovery2"' in html
    assert "/m3-mobile.js?v=20260917-m3-discovery2" in html
