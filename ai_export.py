"""Export the full GovTracker database for offline AI / archive use."""
...
AI_EXPORT_INSTRUCTIONS = """This file is a full snapshot export from GovTracker — a federal contract pipeline tool.

How to use it:
- contracts[]: every opportunity (SAM.gov data, AI analysis, PWS fields, pricing intel, workflow status)
...
- analysis.plain_english_summary: AI screening summary
...
def _attachment_text_for_contract(row: Contract) -> tuple[str | None, str | None]:
    """Return (text, error_note). Downloads PDFs from stored URLs only — no SAM or AI API."""
    try:
...
def build_ai_export(session: Session, *, include_attachment_text: bool = True) -> dict[str, Any]:
    """Assemble a AI-friendly JSON document from the full database."""
    ...
    return {
        "export_format": "govtracker-ai-export",
        "export_version": 1,
        "exported_at": now_utc().isoformat(),
        "instructions_for_ai": AI_EXPORT_INSTRUCTIONS,
...
def export_ai_json_bytes(session: Session, *, include_attachment_text: bool = True) -> tuple[bytes, str]:
    """Return UTF-8 JSON bytes and a suggested filename."""
    payload = build_ai_export(session, include_attachment_text=include_attachment_text)
    stamp = now_utc().strftime("%Y-%m-%d")
    filename = f"govtracker-ai-export-{stamp}.json"
